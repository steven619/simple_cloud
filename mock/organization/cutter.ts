...
for (let i = 1; i < articleCount; i++) {
  ...
  let uuu = []
  for (let j = 1; j < faker.random.number(5); j++) {
    uuu.push({ machine_tool: { name: faker.name.lastName(), id: i }, count: faker.random.number({ min: 1, max: 5 }) })
  }
  articleList.push({
    id: i,
    name: faker.name.lastName(),
    no: 'NO' + faker.random.number({ min: 10000, max: 19999 }),
    cutter_category: cutterCategoryList[index],

    count: faker.random.number({ min: 5, max: 10 }),
    use: uuu
  })
}
