...
for (let i = 1; i < articleCount; i++) {
  let op = fakerZh.Name.findName()
  let wp = faker.name.lastName()
  articleList.push({
    id: i,
    task: {
      name: `[${op}]执行[${wp}]的任务`, id: i
    },
    operator: { name: op, id: i },
    machine_tool: { name: fakerZh.Name.findName(), id: i, machine_tool_category: faker.random.number({ min: 1, max: 11 }) },
    workpiece: { name: wp, id: i },
    count: faker.random.number({ min: 10, max: 70 }),
