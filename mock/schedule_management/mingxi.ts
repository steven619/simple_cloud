...
export const genTask = (i:number) => {
  ...
  return {
    ...
    xinghao:faker.random.number({ min: 100, max: 300 }),
      ...
      diaoboshijian:faker.date.past(),
      danjubianhao:faker.random.number({ min: 100000000, max: 9999999999 }),
      diaoboren:fakerZh.Name.firstName(),
      jincangku:faker.random.number({ min: 0, max: 14 }),
      chucangku:faker.random.number({ min: 15, max: 30 }),
      pingpai:fakerZh.Name.firstName(),
      danjia:faker.random.number({ min: 50, max: 500 }),
  }
}
