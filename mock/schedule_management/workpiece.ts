...
import { articleList as workpieceCategoryList } from './workpiece_category'
...
for (let i = 1; i < articleCount; i++) {
  let index = Math.floor((Math.random() * workpieceCategoryList.length))
  articleList.push({
    id: i,
    name: faker.name.lastName(),
    workpiece_category: workpieceCategoryList[index],
    program_name: faker.name.lastName()
  })
}
